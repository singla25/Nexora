...
jQuery( document ).ready( function ( $ ) {
    ...
    /* Expose globally so marketplace-shell.php can call it */
    window.nexoraInitView = initViewBindings;
            ...
            $.ajax( {
                ...
                success( res ) {
                    if ( res.success ) {
                        ...
                        /* Button success state — green for 2.5 s then resets */
                        $btn.text( '✓ Added to Cart' )
                            .css( { background: 'var(--mk-success)', opacity: '1' } );
                        ...
                        /* Notice with cart link */
                        showNotice(
                            $body,
                            'success',
                            '✓ Product added to cart! ' +
                            '<a href="' + ( res.data.cart_url || M.cart_url || '#' ) + '" ' +
                            'style="font-weight:600;text-decoration:underline;">View Cart →</a>'
                        );
 
                        /* Trigger WC mini-cart fragment refresh */
                        $( document.body ).trigger( 'wc_fragment_refresh' );
 
                        /* Update any visible cart-count badges */
                        const count = res.data.cart_count || 0;
                        $( '.cart-count, .woo-cart-count, .cart-contents-count' ).text( count );
